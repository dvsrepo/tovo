...
object LovFromFileWithTerms {
  ...
  def main(args: Array[String]) {
    ...
    confidenceList.foreach { confidence =>
      ...
      s.close()
      val corpusLovBabelfied = new PrintWriter(new File(s"src/test/corpus/babelfied/terms-lov-senses-corpus-conf-$confidence.txt"))
      corpusLov.foreach {vocab =>
        val (uri, text) = vocab
        val senseTokens = BabelfySenseTermsTokenizer(text, confidence)
        println(s"${uri}\t${senseTokens.mkString(" ")}")
